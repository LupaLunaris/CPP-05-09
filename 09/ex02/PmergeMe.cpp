...
void PmergeMe::parseArgs(int argc, char** argv)
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        // strict conversion + overflow check
        char* end = 0;
...
// ---------------- Jacobsthal order ----------------
// Build insertion order of indices [0..k-1] using Jacobsthal blocks.
// For k=5 -> 0,2,1,4,3  (i.e. 1,3,2,5,4 in 1-based)
static void buildJacobsthalOrderVector(size_t k, std::vector<size_t>& order)
{
    order.clear();
    if (k == 0) return;

    // build increasing Jacobsthal numbers: 1,3,5,11,21...
    ...
    size_t last = 0; // last jacobsthal used (1-based)
    for (size_t a = 0; a < js.size(); ++a)
    {
        size_t j = js[a];
        if (j <= last) continue;
        for (size_t idx = j; idx > last; --idx)
            order.push_back(idx - 1); // to 0-based
        last = j;
    }
                            ...
                            bool& hasOdd, int& odd)
{
    ...
    for (size_t i = 0; i + 1 < in.size(); i += 2)
    {
        int a = in[i];
        int b = in[i + 1];
        if (a < b) pairs.push_back(std::make_pair(a, b)); // (min,max)
...
void PmergeMe::sortVectorFordJohnson(std::vector<int>& v)
{
    ...
    // 2) mainChain = maxes, recursively sorted with same algorithm
    ...
    // 3) insert mins in Jacobsthal order (binary insert, bounded up to its max)
